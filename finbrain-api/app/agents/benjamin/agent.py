"""Tio Patinhas — Benjamin: agente de simulações financeiras educacionais.

Chama skills determinísticas e usa LLM para explicar os resultados.
"""

from decimal import Decimal
from app.skills.calculos import juros_compostos, reserva_emergencia, comparar_rentabilidade, simular_carteira_acoes
from app.guardrails.athena import validar


BENJAMIN_SYSTEM_PROMPT = """Você é Benjamin, simulador financeiro educacional do Tio Patinhas.
...
SIMULATION_TYPES = {
    ...
    "simulacao_acoes": {
        "required": ["valor_inicial", "aporte_mensal", "meses", "tickers_pesos"],
        "skill": "simular_carteira_acoes",
    },
}


def simular(tipo: str, params: dict, anthropic_client=None, stream: bool = False):
    ...
    Args:
        ...
        stream: If True, returns a tuple of (result_dict, explanation_generator).

    Returns:
        Dict with simulation results, explanation, and guardrail status, or (dict, generator) if stream=True.
    """
    ...
    try:
        ...
        elif tipo == "simulacao_acoes":
            result = simular_carteira_acoes(
                valor_inicial=Decimal(str(params.get("valor_inicial", 1000))),
                aporte_mensal=Decimal(str(params.get("aporte_mensal", 0))),
                meses=int(params.get("meses", 12)),
                tickers_pesos=params.get("tickers_pesos", [{"ticker": "PETR4", "peso": 1.0}])
            )
        else:
            result = {}
    ...
    # Guardrails
    athena = validar(explicacao, allow_tickers=(tipo == "simulacao_acoes"))
...
def _fallback_explanation(tipo: str, result: dict) -> str:
    ...
    if tipo == "juros_compostos":
        ...
        return (
            f"📈 **Simulação de Carteira de Ações**\n\n"
            f"Com base no histórico médio fictício, o retorno anual esperado dessa carteira seria de {result.get('retorno_anual_projetado', '?')}%.\n"
            f"Seu saldo projetado ao final do período seria de **R$ {result.get('valor_final', '?')}**.\n\n"
            f"{result.get('premissas', '')}"
        )
